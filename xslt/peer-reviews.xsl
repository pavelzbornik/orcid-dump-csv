...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:for-each select=".//peer-review:peer-review-summary">
    ...
    <xsl:value-of select="peer-review:reviewer-role"/><xsl:text>¬</xsl:text>
    <xsl:value-of select=".//common:external-id/common:external-id-type"/><xsl:text>¬</xsl:text>
    <xsl:value-of select=".//common:external-id/common:external-id-value"/><xsl:text>¬</xsl:text>
    <xsl:value-of select=".//common:external-id/common:external-id-normalized"/><xsl:text>¬</xsl:text>
    <xsl:value-of select=".//common:external-id/common:external-id-normalized/@transient"/><xsl:text>¬</xsl:text>
    <xsl:value-of select=".//common:external-id/common:external-id-url"/><xsl:text>¬</xsl:text>
    <xsl:value-of select=".//common:external-id/common:external-id-relationship"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="peer-review:review-url"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="peer-review:review-type"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="peer-review:completion-date/common:year"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="peer-review:completion-date/common:month"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="peer-review:completion-date/common:day"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="peer-review:review-group-id"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="normalize-space(translate(peer-review:convening-organization/common:name,'[¬]',''))"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="peer-review:convening-organization/common:address/common:city"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="peer-review:convening-organization/common:address/common:region"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="peer-review:convening-organization/common:address/common:country"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="peer-review:convening-organization/common:disambiguated-organization/common:disambiguated-organization-identifier"/><xsl:text>¬</xsl:text>
    <xsl:value-of select="peer-review:convening-organization/common:disambiguated-organization/common:disambiguation-source"/><xsl:text>¬</xsl:text>
