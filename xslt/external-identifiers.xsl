...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:for-each select=".//external-identifier:external-identifier">
    ...
    <xsl:value-of select="normalize-space(translate(common:external-id-type,'[¬]',''))" /><xsl:text>¬</xsl:text>
    <xsl:value-of select="normalize-space(translate(common:external-id-value,'[¬]',''))" /><xsl:text>¬</xsl:text>
    ...
    <xsl:value-of select=".//common:source-name"/><xsl:text>¬</xsl:text>
    <xsl:value-of select=".//common:assertion-origin-name"/>
