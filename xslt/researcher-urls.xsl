...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:for-each select=".//researcher-url:researcher-url">
    ...
    <xsl:value-of select="normalize-space(translate(researcher-url:url-name,'[¬]',''))" /><xsl:text>¬</xsl:text>
    <xsl:value-of select="researcher-url:url"/><xsl:text>¬</xsl:text>
